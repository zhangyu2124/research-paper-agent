...
function describeToolStep(name: string, args: Record<string, any> = {}): string {
  if (args.query && name === "search_papers") {
    const query = args.query.length > 40 ? args.query.substring(0, 40) + "..." : args.query
    return `Searching paper library for "${query}"`
  }
  if (args.query && name === "search_paper_chunks") {
    const query = args.query.length > 40 ? args.query.substring(0, 40) + "..." : args.query
    return `Searching PDF evidence for "${query}"`
  }
  if (name === "get_paper_detail") {
    return "Reading paper metadata"
  }
  if (name === "get_paper_chunk_context") {
    return "Reading surrounding PDF context"
  }
  if (name === "list_papers") {
    return "Listing paper library"
  }
  ...
  const processStream = useCallback(
    async (userContent: string, assistantMessageId: string, images?: ImageAttachment[]) => {
      ...
      const traceMetadata = {
        ...
        source_type: "Research-Paper-Agent",
        graph: agentType,
      }
      ...
      const streamResponse = client.runs.stream(threadId, agentType, {
        ...
        config: {
          ...
          tags: ["Research-Paper-Agent", agentType],
